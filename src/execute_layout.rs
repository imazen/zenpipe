...
fn orient_from(o: Orientation) -> zenresize::OrientOutput {
    ...
    match o {
        Orientation::Identity => OrientOutput::Identity,
        Orientation::FlipH => OrientOutput::FlipH,
        Orientation::Rotate180 => OrientOutput::Rotate180,
        Orientation::FlipV => OrientOutput::FlipV,
        Orientation::Transpose => OrientOutput::Transpose,
        Orientation::Rotate90 => OrientOutput::Rotate90,
        Orientation::Transverse => OrientOutput::Transverse,
        Orientation::Rotate270 => OrientOutput::Rotate270,
        _ => OrientOutput::Identity, // non_exhaustive fallback
    }
}
...
) -> zenresize::SolidBackground {
    match color {
        zenlayout::CanvasColor::Transparent => zenresize::SolidBackground::transparent(desc),
        zenlayout::CanvasColor::Srgb { r, g, b, a } => {
            zenresize::SolidBackground::from_srgb_u8(*r, *g, *b, *a, desc)
        }
        zenlayout::CanvasColor::Linear { r, g, b, a } => {
            zenresize::SolidBackground::from_linear(*r, *g, *b, *a, desc)
        }
        _ => zenresize::SolidBackground::transparent(desc),
    }
}
...
pub fn execute_layout(
    ...
    let (resized, resize_w, resize_h) = if plan.resize_is_identity {
        ...
        let builder = zenresize::ResizeConfig::builder(orient_w, orient_h, rw, rh)
            .filter(filter)
            .format(desc);
...
/// For the primary, get it from [`zenlayout::Pipeline::plan()`]. For a secondary plane,
...
/// The `request` and `ideal` come from [`zenlayout::Pipeline::plan()`] (primary) or
...
pub fn execute_secondary(
    ...
    primary_source: zenlayout::Size,
    secondary_source: zenlayout::Size,
    secondary_target: Option<zenlayout::Size>,
    desc: PixelDescriptor,
    filter: Filter,
) -> Vec<u8> {
...
/// - For [`zenresize::SolidBackground`]: the padding area is filled with the solid color
///   (automatically converted to sRGB u8).
/// - For non-solid backgrounds (e.g. [`zenresize::SliceBackground`]): the padding area
...
pub fn execute_layout_with_background<B: Background + Clone>(
    ...
    let (resized, mut background) = if use_offset {
        ...
        let builder = zenresize::ResizeConfig::builder(orient_w, orient_h, actual_rw, actual_rh)
            .filter(filter)
            .format(desc);
        if let Some(ref data) = oriented {
            let config = builder.build();
...
///
/// Converts the plan's trim (→ [`zenresize::SourceRegion`]), resize target,
/// and canvas placement (→ [`zenresize::Padding`]) into a single config
/// suitable for [`zenresize::StreamingResize`] or [`zenresize::Resizer`].
///
...
/// which automatically applies the orientation via [`zenresize::OrientOutput`].
///
...
pub fn config_from_plan(
    ...
    let mut builder = zenresize::ResizeConfig::builder(decoder_width, decoder_height, rw, rh)
        .filter(filter)
        .format(desc);
...
/// Create a [`zenresize::StreamingResize`] from a [`LayoutPlan`].
///
...
pub fn streaming_from_plan_batched(
    ...
    let orient = orient_from(plan.remaining_orientation);
    zenresize::StreamingResize::with_batch_hint(&config, batch_hint).with_orientation(orient)
}
...
mod tests {
    ...
    fn zero_copy_trim_matches_explicit() {
        ...
        let config_a = zenresize::ResizeConfig::builder(10, 10, 6, 6)
    ...
    fn execute_with_solid_white_bg_fit_pad() {
        ...
        let bg = zenresize::SolidBackground::white(format);
        let result =
    ...
    fn execute_transparent_bg_matches_no_bg() {
        ...
        let transparent_bg = zenresize::SolidBackground::transparent(format);
