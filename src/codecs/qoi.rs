//! QOI codec adapter using zenbitmaps via trait interface.
...
pub(crate) fn probe(data: &[u8]) -> Result<ImageInfo> {
    zenbitmaps::QoiDecoderConfig::new()
        .job()
        .probe(data)
        .map_err(|e| at!(CodecError::from_codec(ImageFormat::Qoi, e)))
}
...
pub(crate) fn decode(
    ...
    let dec = zenbitmaps::QoiDecoderConfig::new();
    let mut job = dec.job();
...
pub(crate) fn build_trait_encoder<'a>(params: EncodeParams<'a>) -> BuiltEncoder<'a> {
    build_from_config(|_p| zenbitmaps::QoiEncoderConfig::new(), params)
}

pub(crate) fn build_streaming(params: EncodeParams<'_>) -> crate::error::Result<StreamingEncoder> {
    crate::dispatch::build_streaming_from_config(|_p| zenbitmaps::QoiEncoderConfig::new(), params)
}
