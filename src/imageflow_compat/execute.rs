...
fn build_pipeline_maybe_traced(
    ...
    let config = if trace_mode == "full" || trace_mode == "svg" {
        crate::trace::TraceConfig::full()
    } else {
        crate::trace::TraceConfig::metadata_only()
    };
    ...
    let (result, mut trace) =
        crate::bridge::build_pipeline_traced(source, nodes, converters, &config)?;
...
fn decode_to_source_frame(
    ...
    for i in 0..=frame_index {
        ...
        if i == frame_index {
            ...
            return Ok(Box::new(crate::sources::MaterializedSource::from_data(
                bytes, w, h, format,
            )));
        }
    }
...
fn jpeg_decode_codec_config() -> zencodecs::config::CodecConfig {
    ...
    #[cfg(not(feature = "nodes-jpeg"))]
    {
        zencodecs::config::CodecConfig::default()
    }
}
...
fn decode_to_source(
    ...
    {
        Ok(streaming) => {
            let source = crate::codec::DecoderSource::new(streaming)?;
            Ok(Box::new(source))
        }
        Err(_) => {
...
fn stream_encode(
    mut source: Box<dyn crate::Source>,
    decision: &zencodecs::FormatDecision,
    encode_io_id: i32,
) -> Result<Vec<ZenEncodeResult>, ZenError> {
    ...
    if !decision.format.supports_alpha() && source.format().has_alpha() {
        ...
        if let Some(op) = crate::ops::RowConverterOp::new_explicit(from, to, &options) {
