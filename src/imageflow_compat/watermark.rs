...
///
/// Uses `crate::watermark::WatermarkLayout` for geometry (bounding box,
...
fn to_watermark_layout(
    ...
    _canvas_w: u32,
    _canvas_h: u32,
) -> crate::watermark::WatermarkLayout {
    use crate::watermark::{FitBox, FitMode, Gravity, WatermarkLayout};
...
pub fn decode_watermark(
    ...
    let target_format = crate::format::RGBA8_SRGB;
    let pixels = if descriptor == target_format {
        ...
        let mat =
            crate::sources::MaterializedSource::from_source(mat_source).map_err(ZenError::from)?;
        mat.data().to_vec()
    };
